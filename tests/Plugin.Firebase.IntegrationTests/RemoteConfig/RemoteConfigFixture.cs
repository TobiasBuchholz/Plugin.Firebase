...
[TestLogging]
[IntegrationTestFixture(IntegrationTestPackage.RemoteConfig)]
...
public sealed class RemoteConfigFixture
{
    ...
    [RealFirebaseFact]
    public async Task gets_keys_for_empty_prefix()
    {
        var sut = CrossFirebaseRemoteConfig.Current;
        var key = IntegrationTestData.UniqueId("acceptance_empty_prefix").Replace('-', '_');

        await sut.SetDefaultsAsync((key, "default"));

        var keys = sut.GetKeysByPrefix("").ToList();

        Assert.Contains(key, keys);
    }

    [RealFirebaseFact]
    public void returns_default_values_for_missing_keys()
    {
        var sut = CrossFirebaseRemoteConfig.Current;
        var key = IntegrationTestData.UniqueId("missing").Replace('-', '_');

        Assert.False(sut.GetBoolean(key));
        Assert.Equal("", sut.GetString(key));
        Assert.Equal(0L, sut.GetLong(key));
        Assert.Equal(0d, sut.GetDouble(key));
    }
